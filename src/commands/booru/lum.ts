...
export default async function lum(client: Client, interaction: Interaction) {
  ...
  const maxPage = 16; // Genuinely just hard-coded, the API doesn't expose this info
  const page = randomInt(1, maxPage);

  fetch(`https://www.sakugabooru.com/post.json?tags=urusei_yatsura+&page=${page}`).then(
    (response) => {
      if (!response.ok) {
      ...
      response
        ...
        .then((body) => {
          ...
          const preview: Post = body[randomInt(0, body.length - 1)];
          ...
          const embed = {
            ...
            author: {
              name: "bt",
              icon_url:
                "https://y5w2vhen03.ufs.sh/f/fPOm9fGnm29kdV2iTG7oflIYyFOEqcSeUpgV96r0TBGnQ1Ak",
